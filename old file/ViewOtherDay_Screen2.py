...
def screen():
    ...
    def setUserInput():
        ...
        global yearValue
        global user_input
        user_input = monthValue  + dayValue +  yearValue
    ...
    MIN = [Min for Min in range (0, 60, 5)]

    Label(window, text = "Choose a Date", font = ("arial 20 bold italic")).pack()
    ...
    user_input = mnthvar.get() + datevar.get()+ yrvar.get()
    global day
    day = datetime.strptime(user_input, '%b%d%Y').strftime('%A')
